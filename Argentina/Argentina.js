...
fetch("/Argentina/Argentina.json")
    ...
    data.forEach(product => {
      ...
      information_team.innerHTML += `
          ...
          <div class="about">
            ...
            <span class="i5">${product.offers.referee}</span>
            <span class="i6">${product.offers.stadium}</span>
            </div>
          </div>
...
async function setupCountdowns() {
  try {
    const response = await fetch('/Argentina/Argentina.json'); // تأكد من تغيير اسم الملف
    if (!response.ok) {
