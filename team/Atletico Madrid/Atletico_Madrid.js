...
fetch("/team/Atletico Madrid/Atletico_Madrid.json")
...
.then(data =>{
    ...
    data.forEach(product => {
      ...
      const isIframeEmpty3 = product.iframe_2   === '';
      const iframeDisplayStyle3 = isIframeEmpty2 ? 'display: none;' : 'display: block;';
      const messageDisplayStyle3 = isIframeEmpty2 ? 'display: block;' : 'display: none;';
      ...
      <div class="video_f v3">
         <h1 style="${messageDisplayStyle3}">لا يوجد</h1>
       <iframe allowfullscreen="true" frameborder="0" height="100%" scrolling="1" 
        src="${product.iframe3}" 
        width="100%">
       </iframe> 
...
async function setupCountdowns() {
  try {
    const response = await fetch('/team/Atletico Madrid/Atletico_Madrid.json'); // تأكد من تغيير اسم الملف
    if (!response.ok) {
