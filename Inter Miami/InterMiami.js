...
fetch("/Inter Miami/InterMiami.json")
...
.then(data =>{
    ...
    data.forEach(product => {
     ...
     <div class="time">
        ...
        <img class="img_time" src="/aimage/Image_fx(35).jpg" alt="">
...
async function setupCountdowns() {
  try {
    const response = await fetch('/Inter Miami/InterMiami.json'); // تأكد من تغيير اسم الملف
    if (!response.ok) {
